...
const iconOptions = [
  ...
  { value: 'thumbs-up', label: 'Polegar', Icon: ThumbsUp },
];
...
const SuccessCasesEditor: React.FC<SuccessCasesEditorProps> = ({ instanceId }) => {
  ...
  const updateCase = (caseId: string, field: keyof SuccessCase, value: string) => {
    const updatedCases = casesConfig.cases.map((caseItem) =>
      caseItem.id === caseId ? { ...caseItem, [field]: value } : caseItem
    );
    updateModuleInstance(instanceId, { cases: updatedCases });
  };
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-[auto_1fr] gap-3">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <Input
                    id={`title-${caseItem.id}`}
                    value={caseItem.title}
                    onChange={(e) => updateCase(caseItem.id, 'title', e.target.value)}
                    placeholder="Ex: Defesa Criminal - 2025"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <Textarea
                  id={`description-${caseItem.id}`}
                  value={caseItem.description}
                  onChange={(e) => updateCase(caseItem.id, 'description', e.target.value)}
                  placeholder="Ex: Cliente acusado injustamente"
              ...
              <div>
                ...
                <Input
                  id={`result-${caseItem.id}`}
                  value={caseItem.result}
                  onChange={(e) => updateCase(caseItem.id, 'result', e.target.value)}
                  placeholder="Ex: Absolvição por falta de provas"
                />
              </div>
            </div>
          </div>
